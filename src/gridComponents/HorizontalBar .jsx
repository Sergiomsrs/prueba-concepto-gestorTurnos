...
export const HorizontalBar = ({
  ...
  const handlePointerDown = (index, e) => {
    e.preventDefault(); // Evita scroll en táctil
    setStartSelection(index);
    setIsSelecting(true);
  };

  const handlePointerEnter = (index, e) => {
    // Solo arrastra si está presionado un botón o dedo
    ...
    if (isSelecting && startSelection !== null) {
      ...
      const isStartSelected =
        hours[startSelection] !== "Null" || hours[startSelection] === "PTO";
  ...
  return (
    <>
      <td className="sm:text-base text-sm font-semibold text-gray-800 py-0 whitespace-nowrap">
        {teamWork}
      </td>
      <td className="sm:text-base text-sm font-semibold text-gray-800 whitespace-nowrap max-w-none">
        <button onClick={handleOpenModal} className="mr-2 py-0">
          {username} {lastName}
        </button>
      </td>
      ...
      {hours &&
        hours.map((value, index) => (
          <td
            className={`relative md:w-24 w-12 text-center align-middle py-0 ${
              index % 4 === 3 ? "border-45-height" : ""
            }`}
            key={index}
          >
            <input
              ...
              onChange={(event) =>
                handleClick(index, event.target.checked)
              }
            ...
            {index % 4 === 3 && (
              <div
                className="absolute right-0 top-1/2 h-1/2 bg-slate-400"
                style={{ zIndex: -1, width: 1 }}
              ></div>
            )}
          </td>
        ))}

      <td className="sm:text-base text-xs w-12 px-2 align-middle py-0.5 ">
        {formatTime(shiftDurationes)}
      </td>
