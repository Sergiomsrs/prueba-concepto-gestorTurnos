...
export const SideBar = ({ sideBarClick, isOpen }) => {
  ...
  const [isShiftMenu, setIsShiftMenu] = useState(false);
  const [date, setDate] = useState({ start: '', end: '' });
  const [error, setError] = useState({ start: '', end: '' });
  const { handleCopyWeek, shiftMessage } = useShift();
  ...
  const sideBarMenu = () => {
    setIsShiftMenu(false)
    setIsOpenMenu(!isOpenMenu);
  }
  const handleShiftBarMenu = () => {
    setIsOpenMenu(false)
    setIsShiftMenu(!isShiftMenu);
  }

  const handleSave = async (e) => {
    e.preventDefault();
    try {
      await handleCopyWeek({
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (event) => {
      if (sidebarRef.current && !sidebarRef.current.contains(event.target)) {
        sideBarClick(); // Llama a la función para cerrar la barra
      }
    };
      ...
      <div className="flex flex-col justify-between flex-1 mt-6">
        <nav className="-mx-3 space-y-3">
          ...
          >
            ...
            <span className="mx-2 text-sm font-medium">Añadir turno</span>
          </button>
