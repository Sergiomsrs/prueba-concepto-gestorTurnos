...
export const newProcessTimeStamps = (dailySchedules, id) => {
    ...
    return dailySchedules.map(dayData => {
        ...
        const hasMidnight = times.some(t =>
            t.timestamp.getHours() === 0 && t.timestamp.getMinutes() === 0
        );

        times.sort((a, b) => hasMidnight
            ? a.id - b.id
        ...
        for (let i = 0; i < times.length; i += 2) {
            ...
            if (i + 1 < times.length) {
                ...
                const isMidnightExit = exit.timestamp.getHours() === 0 && exit.timestamp.getMinutes() === 0;
                const crossesMidnight = isMidnightExit || exit.timestamp < entry.timestamp;

                if (crossesMidnight) {
                    const minutesToMidnight = (24 * 60) - (entry.timestamp.getHours() * 60 + entry.timestamp.getMinutes());
                    const minutesFromMidnight = exit.timestamp.getHours() * 60 + exit.timestamp.getMinutes();
                    periodMs = (minutesToMidnight + minutesFromMidnight) * 60 * 1000;
                } else {
                    periodMs = exit.timestamp - entry.timestamp;
                }
